...
S3へのアクセスをCloufFront経由に制限する機能。<br>
OACを利用することで、バケットポリシーよりも柔軟なアクセス制御とセキュリティ強化ができる。
- オリジン（S3）に対して、署名付きリクエスト（SigV4）を送信し、S3がその署名を検証する（認証）
  - リクエスト情報（メソッド、URI、ヘッダ、ボディなど）を整形してハッシュ化（Canonical Request）
  - そのハッシュ値を元に署名を生成し、Authorization ヘッダに付与
- IAM統合によるバケットポリシーでのアクセス制御（認可）
  - CloudFrontはS3バケットポリシーにて IAMサービスプリンシパル（cloudfront.amazonaws.com）として評価される
- 包括的な HTTP メソッドのサポート
- ディストリビューション単位でアクセス制限できる
