...
<br><br>
## CloudFrontにおけるキャッシュキーの役割
CloudFrontをはじめとするCDNでは、同一のリクエストに対して同じキャッシュレスポンスを返すことが望まれる（＝キャッシュヒット率の向上につながるため）。<br>
...
- キャッシュととオリジンリクエストはそれぞれ別のポリシーを使って制御するが関連性がある。
- キャッシュキーに含めた項目（ヘッダ・クエリ・Cookie）は、自動的にオリジンリクエストにも含まれる。
- キャッシュキーには含めず、オリジンにだけ渡したい値を制御できる（オリジンリクエストポリシー）
- オリジンリクエストポリシーに含めた値は、キャッシュキーには影響しない。
- キャッシュキーに使ってもオリジンに渡さない、という構成はできない（仕様的に）。
- 逆に、オリジンにだけ渡してキャッシュには影響させたくない場合は、キャッシュポリシーには含めず、オリジンリクエストポリシーにのみ含める。
<br><br>
## キャッシュポリシー（Cache Policy）
CloudFrontがレスポンスを“キャッシュするかどうか”を決めるルールセット。<br>
- キャッシュキーは、キャッシュ内のオブジェクトごとの一意の識別子であり、Viewerリクエストがキャッシュヒット率を左右する。
- キャッシュキーに含める項目は、HTTPヘッダー、Cookie、およびクエリ文字列を指定できる。
- Viewerリクエストが以前のリクエストと同じキャッシュキーを生成し、キャッシュキーのオブジェクトがエッジロケーションに存在し、かつ有効な場合にキャッシュヒットする。
- キャッシュキーに含まれる値が少なくなるとキャッシュヒット率が高まる（ウェブサイトやアプリケーションのパフォーマンスが高くなる）
- キャッシュキーを管理するには、キャッシュポリシーを使用する
- キャッシュポリシーは、CloudFrontディストリビューションの1つ以上のキャッシュ動作にアタッチする
- キャッシュポリシーを使用してキャッシュ内のオブジェクトの有効期限 (TTL) 設定が指定でき、圧縮オブジェクトのリクエストおよびキャッシュができるようになる。
### ■ Time to live (TTL) 設定
cache-Control および Expires HTTPヘッダー（オリジンレスポンス内にある場合）と連動して、CloudFrontキャッシュ内のオブジェクトの有効期間を決定する。
### ■ キャッシュキー設定
キャッシュキーに含めるビューワーリクエストの値を指定。
- 値には、URLクエリ文字列、HTTPヘッダー、Cookieを含めることができる。
- キャッシュキーに含める値は、オリジンリクエストと呼ばれるに自動的に含まれる。
### ■ 圧縮設定
Gzip または Brotli圧縮形式で圧縮されたオブジェクトをリクエストおよびキャッシュできる (ビューワーでサポートされている場合)。
- ViewwerはAccept-EncodingHTTP ヘッダーを使用して圧縮形式のサポートの可否を示す。
<br><br>
## オリジンリクエストポリシー（Origin Request Policyy）
キャッシュミスが発生した場合、CloudFrontはオブジェクトを取得するためのリクエストをオリジンに送信する (オリジンリクエスト)。<br>
オリジンリクエストには、ビューワークエストの次の情報が常に含まれる。
- URLパス (URL クエリ文字列またはドメイン名を含まないパスのみ)
- リクエストボディ (存在する場合)
- CloudFrontがすべてのオリジンリクエストに自動的に含めるHTTPヘッダー (Host、User-Agent、X-Amz-Cf-Id など)
  - Host：
  - User-Agent
  - X-Forwarded-For
  - X-Forwarded-Proto
