...
Viewerリクエストが以前のリクエストと同じキャッシュキーを生成し、キャッシュキーのオブジェクトがエッジロケーションに存在し、かつ有効な場合にキャッシュヒットする。<br>
キャッシュキー管理には、CloudFrontがレスポンスを“キャッシュするかどうか”を決めるルールセット＝キャッシュポリシーを使用する。<br>
- キャッシュキー設定：キャッシュキーは、キャッシュ内オブジェクトごとの一意の識別子であり、Viewerリクエストのキャッシュヒット率を左右する。
  - 値には、URLクエリ文字列、HTTPヘッダー、Cookieを含めることができる。
  - キャッシュキーに含める値は、オリジンリクエストに自動的に含まれる。
- TTL設定：cache-Control や Expires HTTPヘッダーと連動して、キャッシュ内オブジェクトの有効期間を指定する。
