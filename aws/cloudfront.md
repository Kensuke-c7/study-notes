...
#### ・コンテンツ圧縮：オリジンから取得した非圧縮コンテンツを、エッジでGzip/Brotli圧縮してキャッシュ・配信可能。
#### ・高帯域・低レイテンシの専用バックボーンの利用（AWSグローバルバックボーンネットワーク）
### 1.2.2 セキュリティ・アクセス制御系
#### ・HTTPS通信対応：ACM（無料DV証明書）やカスタム証明書（OV/EV）を利用したHTTPS通信
#### ・接続プロトコル制御：HTTP→HTTPS自動リダイレクトやTLSバージョンなどを制御（Viewer Protocol Policy、Origin Security Policy ）
#### ・オリジンアクセスコントロール (OAC)：S3への直接アクセスを防ぎ、CloudFront経由のみでアクセスさせる。
#### ・署名付きURL/Cookie：アプリ側で署名付きURL/Cookieを発行し、CloudFrontが検証することで認証済みユーザのみに限定コンテンツを配信可能。
#### ・地理的制限（Geo-restriction）：特定の国や地域レベルでのアクセスをブロック
#### ・サイト保護：AWS WAFにWeb Application Firewall、AWS ShiledによるDDoS対策
#### ・フィールドレベル暗号化サポート（Field-Level Encryption）
...
#### ・標準アクセスログ：ディストリビューションごとにS3バケットへ配信ログを保存できる（標準ログは最大数分遅延）
#### ・リアルタイムログ：Kinesis Data Streamsと連携してリアルタイムでリクエストログを取得（秒単位の詳細ログ）
#### ・メトリクス監視：CloudWatchに標準メトリクスを送信可能（キャッシュヒット率、エラー数、転送量）
#### ・アクセス制限レポート：WAFログを活用して詳細なブロック状況を分析可能
### 1.2.6 課金
...
#### ・キャッシュポリシー：キャッシュキーを決定するポリシー。
#### ・オリジンリクエストポリシー：オリジンに転送するリクエスト情報を制御するポリシー。
#### ・レスポンスヘッダポリシー：レスポンスに付与するヘッダを定義するポリシー
<br><br>
...
### ■ 全体設定(共通設定)
- ディストリビューション名
- ドメイン名（xxxx.cloudfront.net）：CloudFrontが発行するデフォルトのFQDN
- 代替ドメイン名（CNAME）：独自ドメインを指定 ※オプション
- SSL証明書：ACMの証明書 or インポート証明書
- カスタムSSL設定：SNI or レガシーIP対応
- セキュリティポリシー：ビューワー (クライアント) との HTTPS 接続に使用するセキュリティポリシー
- HTTPバージョン：ビューワーと通信するときにサポートするHTTPバージョン
- 標準ログ記録：CloudWatch Logs、S3などの配信先にログを送信
- AWS WAF ウェブ ACL：連携するAWS WAFの指定 ※オプション
- 価格クラス：Price Class設定（エッジリージョンの選択）
- 地理的制限（Geo-restriction）
### ■ オリジン設定
### ■ ビヘイビア設定（CloudFront設定のキモ）
### ■ 可用性・障害時設定
- オリジングループ：フェイルオーバー設定
- カスタムエラーレスポンス：エラー時の代替レスポンス設定
### ■ その他
- IPv6サポート：有効/無効の切替
- cookieログ作成：Cookieをアクセスログに含めるようにする
<br><br>
...
- 独自のアクセス制御機能（S3署名付きURL、OAC、S3バケットポリシー）
...
#### ・ オリジン：転送先オリジン（S3、ALB、EC2、API Gateway、Lambda関数など）
### ■ キャッシュ制御（どうキャッシュするか）
#### ・ キャッシュポリシー：キャッシュキーとキャッシュTTLの設定（マネージド or カスタムから選択）
#### ・ 圧縮設定：Gzip/Brotli圧縮の使用（※オリジンが非圧縮の場合でもエッジで圧縮可能）
### ■  オリジン転送制御系（オリジンに何を送るか）
#### ・ オリジンリクエストポリシー：オリジンへ転送するヘッダ、Cookie、クエリ文字列などの制御（マネージド or カスタムから選択）
#### ・ 追加ヘッダー：オリジンへの追加ヘッダ送信設定（例：認証トークンやカスタム属性）
### ■ アクセス制御（誰に・どうアクセスさせるか／させないか）
#### ・ Viewerプロトコルポリシー：ViewerからのHTTP/HTTPS接続制御（例：HTTP→HTTPS自動リダイレクト）
#### ・ HTTPメソッド制御：許可するHTTPメソッド（GET、HEAD、OPTIONS、PUT、POST、PATCH、DELETE）
#### ・ 署名付きURL/Cookie：署名検証によるコンテンツ保護（Key Group or IAM key を使用）
### ■ スポンス処理（レスポンスパケットに手を加えるか）
#### ・ レスポンスヘッダポリシー：セキュリティヘッダ、CORSヘッダ、キャッシュヘッダの付与設定（マネージド or カスタムから選択）
### ■ カスタム処理（リクエスト／レスポンス時の高度な処理を追加）
#### ・ Lambda@Edge：Python/Node.jsコードを実行。Viewer/Originの各フェーズでフル機能実行可能。
#### ・ CloufFront Function：超軽量・高速なエッジ処理（Viewer Request/Responseで使用）
### ■ ログ設定（ビヘイビア単位での標準アクセスログ出力設定）
#### ・ ビヘイビア単位でログのON/OFFを設定可能（例：ビヘイビアAはログ出力、ビヘイビアBはOFF）
<br><br>
...
2. そのキャッシュキーに対応するキャッシュオブジェクトの存在有無を確認
3. 有効なキャッシュが存在すれば、レスポンスとして返す
4. 存在しない場合、オリジンサーバーへリクエストを転送
5. 取得したレスポンスを、同じキャッシュキーに紐づけてキャッシュとして保存（TTLなどの設定に従う）
<br>
