...
- ALBは1つのリスナーに対して複数のルーティングルールを定義可（1:N構成）
### リスナールール、条件
リスナーに対して１つまたは複数のルール（条件分岐）を設定し、ルーティング先のターゲットグループを選択できる。
条件を満たすと対応するアクションを呼び出す。
- ホストヘッダー：設定したホスト名（例：*.example.com）に合致するリクエストに対してアクションを行う
- パスパターン：パスパターン（例：/img/*）に合致するリクエストに対してアクションを行う
...
### アクション
リスナールールに合致した際のアクション。
- フォワード：１つまたは複数のターゲットグループにリクエストをフォワード。
- 認証：Cognito認証やOIDC認証 ※HTTPSリスナーのみサポート
- 固定レスポンス：
...
- TCP/UDP/TLSトラフィックをルーティング（IPアドレス＋ポート番号に基づく分散）
  - フロー（IPアドレス＋ポート番号＋プロトコル）ごとに選択されたターゲットに転送（TCPハッシュベースルーティング）
- 超高スループット：数百万リクエスト／秒の処理が可能
- 送信元IPの書き換えなし（クライアントIPは保持される）※条件によってはSNAT可
...
### ヘルスチェック
- ヘルスチェック対象：ターゲットグループに登録された各ターゲット（EC2/IP）
