...
- サブネット毎に１つのVPCルータ（CIDR +1 アドレス）とDNSリゾルバ（CIDR +2 アドレス）を持つ。
...
## ＳＧ（セキュリティグループ）：ＥＮＩにアタッチするステートフルファイアウォール
...
## ＮＡＣＬ（ネットワークアクセスコントロール）：サブネットのアクセスリスト
...
## ＤＮＳ（AmazonProvidedDNS）：サブネット内のＤＮＳリゾルバ
...
## インターネットゲートウェイ（ＩＧＷ）：インターネット接続用の出入口
...
## NATゲートウェイ（ＮＧＷ）：NAT機能を持つマネージドサービス
- プライベートサブネットのインスタンスからインターネットに出るためのターゲット
- サブネット単位で配置する
  - パブリックNATゲートウェイ：パブリックサブネットに配置し、Elastic IPを割り当てる
  - プライベートNATゲートウェイ：プライベートサブネットに配置（インターネット通信不可、他VPC接続用）
- アウトバウンド通信専用（インバウンド通信のNATは非対応）
- AWSフルマネージドサービス（AZ内で冗長化） ※AZ間冗長は利用者側で設定必要
<br><br>

## バーチャルプライベートゲートウェイ（ＶＧＷ）：オンプレ接続用の出入口
- オンプレミスや他VPC（Direct Connect/VPN経由）への送信ターゲットとして指定
- VPC単位でアタッチする
- 1つのVPCに対して1つだけアタッチ可能
<br><br>

## トランジットゲートウェイ（ＴＧＷ）：複数のオンプレやＶＰＣを相互接続する中央ハブサービス
- 複数VPC/オンプレを1対多で接続できる（TGWが中央ハブとして機能）
- 接続先同士の柔軟な通信制御ができる
  - 専用ルートテーブルで制御（TGWアタッチメントルートテーブル）
- アタッチメント単位で接続する（TGWアタッチメント）
  - アタッチメントは各VPC（のいずれかのサブネット１つ）に配置する 
- フルマネージドサービス
- 超高速帯域（数十Gbps単位で自動スケール）
- 通信フロー（VPCｰA → オンプレ（DXGW経由））
  ① サブネットルートテーブルで（同一VPC内の）TGWアタッチメントをターゲットとして指定
  ② TGWアタッチメントテーブルで（宛先VPCの）TGWアタッチメントをターゲットとして指定
  ③ （TGWアタッチメントの配置された）サブネットルートテーブルでVGWをターゲットとして指定
